...
// Attempts a connection using the given parameters and immediately drops it -
// never touches the connection store, the keychain, or `AppState`'s active
// connection. A successful `Config::connect` already proves the startup
// handshake (including authentication) succeeded, so there's nothing more to
// do with the connection once we have it.
#[tauri::command]
