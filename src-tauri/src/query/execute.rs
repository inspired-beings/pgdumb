...
pub const MAX_ROWS_PER_STATEMENT: usize = 1_000;
...
pub async fn execute(client: &Client, sql: &str) -> Result<Vec<StatementOutcome>, String> {
    ...
    let stream = client
        ...
        .await
        .map_err(|e| describe_postgres_error(&e))?;
    ...
    while let Some(message) = stream.next().await {
        match message {
            ...
            Ok(SimpleQueryMessage::Row(row)) => {
                ...
                for i in 0..row.len() {
                    let value = row.try_get(i).map_err(|e| describe_postgres_error(&e))?;
            ...
            Ok(_) => {}
            Err(e) => {
                builder.on_error(describe_postgres_error(&e));
                break;
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn row_cap_matches_the_documented_default() {
        assert_eq!(MAX_ROWS_PER_STATEMENT, 1_000);
    }
